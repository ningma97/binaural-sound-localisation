...
for n = 1:size(fbIdx1,1)
    ...
    for m = 1:2
        %if min(abs(azimuths(pIdx2) - fbAzNew(m))) > 5
        %if distribution2(azimuths==fbAzNew(m)) < threshold
        if min(calc_azimuth_distance(fbAzNew(m), azimuths(pIdx2))) > 6
...
for n = 1:size(fbIdx2,1)
    ...
    for m = 1:2
        ...
        if sum(fbAz2(n,mod(m,2)+1)==srcAz)>0 || ...
        	min(calc_azimuth_distance(fbAzNew(m), azimuths(pIdx1))) > 6 %distribution1(azimuths==fbAzNew(m)) < threshold
