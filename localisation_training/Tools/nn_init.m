function nn = nn_init(architecture, gauss_weight, neg_bias)
...
%   gauss_weight : If true, use gaussian noise for weights (default true)
%       neg_bias : If true, use negative bias uniform [-4.1,-3.9] (default true)
%
%
% Ning Ma, 29 Jan 2015
%
...
for layer = 2 : nn.n   
    ...
    % the weight matrix
    if gauss_weight
        % Initialise weights as Gaussian random variables with mean 0 and
        % standard deviation 1/sqrt(N_in)
        %gauss_scale = 1 / sqrt(nn.size(layer-1));
        gauss_scale = 0.1;
    ...
    if layer == nn.n % layer before softmax -> use zero
        nn.W{layer-1}(:,1) = 0;
    elseif neg_bias
        %nn.W{layer-1}(:,1) = randn(nn.size(layer), 1);
